...
DENY Rules:
  ...
  - Pattern: material update started without a current-state baseline check and persistent baseline `dai docs build [target]`
  - Pattern: material checkpoint without canonical-state update followed by persistent `dai docs build [target]`
  - Pattern: handoff or completion without strict `dai docs gate [target]` and final persistent `dai docs build [target]`

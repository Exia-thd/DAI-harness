...
def _can_symlink(directory: Path) -> bool:
    ...
    Windows requires Developer Mode or SeCreateSymbolicLinkPrivilege, so these
    symlink-escape fixtures cannot be built there at all.
    """
...
def test_catalog_symlink_source_path_is_never_verified(tmp_path: Path) -> None:
    ...
    if not _can_symlink(linked.parent):

...
# These hook commands are POSIX shell. `/bin/bash` is not a real path on
# Windows, so resolve the interpreter the host actually provides (Git Bash
# supplies one) and skip only when there is none.
...
def test_lifecycle_context_hooks_fail_open_when_interpreter_is_missing(
    ...
    command_path.mkdir()
    # A stripped PATH is built from symlinks; Windows needs Developer Mode
    # or SeCreateSymbolicLinkPrivilege to create one at all.
    try:

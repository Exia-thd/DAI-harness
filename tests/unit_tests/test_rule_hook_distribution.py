...
# These lifecycle hook commands are POSIX shell. `shell=True` would hand them
# to cmd.exe on Windows, so run the host's bash explicitly instead.
BASH = shutil.which("bash") or ("/bin/bash" if os.path.exists("/bin/bash") else None)


def _hook_path(path: Path | str) -> str:
    """Spell a path the way the installer and doctor write it into config.

    Both canonicalize through `cygpath -m` so the native hosts that read these
    configs get C:/... rather than an MSYS /c/... they cannot open. That is a
    forward-slash spelling, not the backslashes `str(Path)` produces here.
    """
...
def _paths_agree(needle: Path | str, haystack: str) -> bool:
    return _slash(needle) in _slash(haystack)
...
def assert_context_command(hook: dict, framework: Path, event: str) -> None:
    ...
    assert _paths_agree(hook_path, command)
...
def test_installer_distributes_rule_runtime_and_preserves_user_hooks(
    ...
    assert _paths_agree(runtime, codex)
...
def test_installer_migrates_old_codex_context_path_without_duplicates(
    ...
    codex = codex.replace(
        _hook_path(framework / "scripts" / "lite" / "rule-context-hook.py"),
        _hook_path(framework / "rule-context-hook.py"),
    )
    ...
    assert _hook_path(framework / "rule-context-hook.py") not in result
    assert result.count(" CODEX SessionStart || true") == 1
    assert result.count(" CODEX SubagentStart || true") == 1
    assert _hook_path(framework / "scripts" / "lite" / "rule-context-hook.py") in result
...
def test_hook_doctor_preserves_unrelated_codex_verify_hook(tmp_path: Path) -> None:
    ...
    codex_path.write_text(
        codex_path.read_text(encoding="utf-8").replace(
            f"bash {_hook_path(framework / 'scripts' / 'lite' / 'stop-gate.sh')} --platform CODEX",
            unrelated,
        ),
        encoding="utf-8",
    )
    ...
    assert fixed.returncode in (0, 1), fixed.stdout + fixed.stderr
    result = codex_path.read_text(encoding="utf-8")
    assert f'command = "{_hook_path(unrelated)}"' in result
    assert (
        f'command = "bash {_hook_path(ROOT / "scripts" / "lite" / "stop-gate.sh")} --platform CODEX"'
        in result
    )
...
def test_hook_doctor_migrates_trusted_legacy_codex_verify_hook(tmp_path: Path) -> None:
    ...
    codex = codex_path.read_text(encoding="utf-8").replace(
        f"bash {_hook_path(framework / 'scripts' / 'lite' / 'stop-gate.sh')} --platform CODEX",
        f"bash {_hook_path(framework / 'verify-gate.sh')} --platform CODEX",
    )
    codex_path.write_text(codex, encoding="utf-8")
    env = os.environ.copy()
    env.update(
...
def test_codex_legacy_migration_removes_orphan_parent_timeout(tmp_path: Path) -> None:
    ...
    codex_path.write_text(
        ...
        '[[hooks.Stop]]\nmatcher = "*"\n[[hooks.Stop.hooks]]\n'
        'type = "command"\ncommand = "bash '
        + _hook_path(framework / "verify-gate.sh")
        + ' --platform CODEX"\ntimeout = 2\n',
        encoding="utf-8",
    )
    installed = run_installer(home, framework)

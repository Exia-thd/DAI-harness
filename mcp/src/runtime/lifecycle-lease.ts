...
/** One process and its parent, from a single PowerShell launch.
 *
 * Starting PowerShell costs well over a second here, and lease reconciliation
 * inspects every stored lease at startup - so asking twice per process turned
 * a boot into tens of seconds. Ask once and return both.
 */
async function windowsProcessFacts(
  pid: number,
): Promise<{ current: RawProcessFacts; parentStartedAt: string } | null> {
  const script =
    `$ErrorActionPreference='Stop';` +
    `$p = Get-CimInstance Win32_Process -Filter "ProcessId=${pid}";` +
    `if (-not $p) { exit 1 };` +
    `$parent = $null;` +
    `if ($p.ParentProcessId -gt 0) { $parent = Get-CimInstance Win32_Process -Filter "ProcessId=$($p.ParentProcessId)" -ErrorAction SilentlyContinue };` +
    `$parentStart = '';` +
    `if ($parent) { $parentStart = $parent.CreationDate.ToString('o') };` +
    `'{0}|{1}|{2}|{3}' -f $p.CreationDate.ToString('o'), $p.ParentProcessId, $parentStart, $p.CommandLine`;
  ...
  const [startedAt, parent, parentStartedAt, ...rest] = stdout.trim().split('|');
  const command = rest.join('|');
  const parentPid = Number.parseInt(parent, 10);
  if (!startedAt || !command || !Number.isSafeInteger(parentPid)) return null;
  // Windows has no process groups. `pgid` only ever participates in the
  // identity equality check here - it is never used to signal a group - so
  // binding it to the pid keeps the identity exact without inventing one.
  return {
    current: { pidStartedAt: startedAt, pgid: pid, parentPid, command },
    parentStartedAt: parentStartedAt ?? '',
  };
}

async function posixProcessFacts(pid: number): Promise<RawProcessFacts | null> {
  ...
  return { pidStartedAt, pgid, parentPid, command };
}

class SystemProcessInspector implements ProcessInspector {
  async inspect(pid: number): Promise<ProcessIdentity | null> {
    if (!Number.isSafeInteger(pid) || pid <= 0) return null;
    // `ps` does not exist on Windows; without a native equivalent the store
    // would report every process as unidentifiable and the server would
    // refuse to start rather than degrade its ownership guarantee.
    try {
      if (process.platform === 'win32') {
        const facts = await windowsProcessFacts(pid);
        if (facts === null) return null;
        return {
          pid,
          pidStartedAt: facts.current.pidStartedAt,
          pgid: facts.current.pgid,
          parentPid: facts.current.parentPid,
          parentStartedAt: facts.parentStartedAt,
          commandDigest: digest(facts.current.command),
        };
      }
      const current = await posixProcessFacts(pid);
      if (current === null) return null;
      const parentStartedAt =
        current.parentPid > 0
          ? ((await posixProcessFacts(current.parentPid))?.pidStartedAt ?? '')
          : '';
      return {
        pid,
        pidStartedAt: current.pidStartedAt,
        pgid: current.pgid,
        parentPid: current.parentPid,
        parentStartedAt,
        commandDigest: digest(current.command),
      };
    } catch {
      return null;
    }
...
export class LifecycleLeaseStore {
  ...
  private async atomicWrite(lease: LifecycleLease): Promise<void> {
    ...
    // The rename above is already atomic. Syncing the containing directory is
    // an extra durability barrier that Windows does not offer: opening a
    // directory handle, or fsync on one, fails with EPERM/EISDIR there.
    try {

...
from evidence_common import fchmod, lock_exclusive, unlock
...
def _read_bounded(path: Path, limit: int) -> bytes | None:
    ...
    if limit < 0:
        return None
    # os.open defaults to text mode on Windows, which translates CRLF and
    # would digest bytes the file does not contain.

...
_SUPPORTS_DIR_FD = all(
    getattr(os, name, None) in getattr(os, "supports_dir_fd", set())
...
def _read_bounded_regular(path: Path, limit: int) -> bytes | None:
    ...
    try:
        ...
        if not stat.S_ISREG(initial.st_mode) or initial.st_size > limit:
            return None
        # Without O_NOFOLLOW the lstat above plus the fstat identity check
        # below are what reject a symlink, including one swapped in
        # between the two calls.
        fd = os.open(
            path,
            os.O_RDONLY
            | getattr(os, "O_BINARY", 0)
            | (os.O_NOFOLLOW if HAS_NOFOLLOW else 0)
...
def _atomic_json(path: Path, value: dict[str, Any]) -> None:
    ...
    if not HAS_NOFOLLOW or not _SUPPORTS_DIR_FD:
        _atomic_json_pathwise(path, payload)
        return
...
def _atomic_json_pathwise(path: Path, payload: bytes) -> None:
    """Atomic replace for hosts without directory descriptors or O_NOFOLLOW.

    Windows has neither, so anchor on the parent directory by path instead and
    refuse a symlinked target. ``os.replace`` is still atomic there; only the
    directory fsync barrier is unavailable.
    """

    directory = path.parent
    if is_symlink(directory) or is_symlink(path):
        raise OSError("refusing to write retry state through a symlink")
    temp: Path | None = None
    fd = -1
    try:
        for _ in range(100):
            candidate = (
                directory / f".{path.name}.{next(tempfile._get_candidate_names())}"
            )
            try:
                fd = os.open(
                    candidate,
...
def _retry_state(root: Path, scope: str, key: str, *, record: bool) -> tuple[bool, str]:
    ...
    try:
        ...
        if not stat.S_ISREG(lock_info.st_mode):
            return False, "validation_failed"
        fchmod(lock_fd, 0o600, lock_path)
...
def main() -> int:
    ...
    # This tree's stub check is workspace-anchored rather than cwd-relative.
    stub_errors = _check_stubs(root, files)
    if stub_errors:

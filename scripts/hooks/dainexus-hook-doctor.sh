...
# Under MSYS/Git Bash, `pwd` yields /c/... while every native tool this script
# calls - node, and the configs it writes - speaks C:/.... Resolve to the one
# spelling both understand, or the project's own gate stops being recognizable
# as trusted and repairs point at the framework install instead.
canonical_path() {
...
FRAMEWORK_DIR="$(canonical_path "${DAINEXUS_DIR:-${HOME}/.dainexus}")"
...
SOURCE_ROOT="${DAINEXUS_SOURCE_DIR:-$(cd "${SCRIPT_DIR}/../.." && pwd)}"
...
rule_context_doctor_json() {
    ...
    local result
    if result="$(RULE_CONTEXT_COMMAND="$command" RULE_CONTEXT_COMMAND_2="$command2" DAINEXUS_DIR="$FRAMEWORK_DIR" node - "$platform" "$file" "$auto_fix" <<'NODE'
const fs = require('fs');
const path = require('path');
...
function ownedContextCommand(candidate, event) {
  ...
  if (typeof candidate !== 'string' || candidate.indexOf('DAINEXUS_RULE_HOOK_MODE=') !== 0 || candidate.indexOf('hook_path="$1"; fallback_workspace="$2"; platform="$3"; event="$4";') === -1) return false;
  const marker = " -- '";
  ...
  const framework = path.resolve(process.env.DAINEXUS_DIR);
...
if (platform === 'ANTIGRAVITY') {
  const policy = cfg['dai-nexus-policy'] && typeof cfg['dai-nexus-policy'] === 'object' && !Array.isArray(cfg['dai-nexus-policy']) ? cfg['dai-nexus-policy'] : {};
...
rule_context_doctor_codex() {
    ...
    local auto_fix="$4"
    local result
    if result="$(CONTEXT_SESSION_CMD="$session_command" CONTEXT_SUBAGENT_CMD="$subagent_command" DAINEXUS_DIR="$FRAMEWORK_DIR" python3 - "$file" "$auto_fix" <<'PYEOF'
import json
import os
import re
import sys
...
# security policy hooks.  Diagnosis only inspects the exact DAI Nexus entry;
...
PROJECT_ROOT="$(canonical_path "$(cd "${SCRIPT_DIR}/../.." && pwd)")"
...
if (!BT.some(function(g){ return Array.isArray(g.hooks) && g.hooks.some(function(h){ return h && (h.command === 'bash ' + process.env.FRAMEWORK_BEFORE_GATE || h.command === 'bash ' + process.env.PROJECT_BEFORE_GATE); }); })) BT.push({ matcher: '*', hooks: [{ name: 'dai-nexus-policy', type: 'command', command: 'bash $PROJECT_ROOT/scripts/lite/gemini-before-tool-gate.sh', timeout: 5000 }] });
    ...
    if [[ "$codex_schema" == "true" ]]; then
        ...
        if [[ "$AUTO_FIX" == "true" ]]; then
            ...
            if [[ -z "$PYTHON_BIN" ]]; then
                ...
                DAINEXUS_DIR="$FRAMEWORK_DIR" DAINEXUS_PROJECT_ROOT="$PROJECT_ROOT" "$PYTHON_BIN" - "$CODEX_CONFIG" <<'PYEOF'
import os
import re
import shlex
import sys
...
def owned_legacy_verify(command: str) -> bool:
    ...
    roots = [
        Path(os.environ["DAINEXUS_DIR"]).resolve(),
        Path(os.environ["DAINEXUS_PROJECT_ROOT"]).resolve(),
    ]
    trusted = {
        root / "verify-gate.sh"
        for root in roots

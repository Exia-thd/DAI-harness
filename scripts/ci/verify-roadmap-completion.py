...
def _resolve_interpreter(command: list[str]) -> list[str]:
    ...
    On Windows a bare `python3` resolves to the Microsoft Store alias stub,
    which exits non-zero without running anything, and `npm`/`npx` exist only
    as `.CMD` shims that subprocess will not find without an extension - so a
    manifest command naming either would report every deliverable as failing
    for a reason that has nothing to do with the deliverable. Resolve the
    launcher; the rest of the argv stays exact.
    """
...
def _run_verifier(
    ...
    command = _resolve_interpreter(verification["command"])
...
def _clone_workspace(destination: Path) -> tuple[Path, str]:
    ...
    if sys.platform == "darwin":
        ...
        # Windows reached the shutil fallback, which copies a 469MB worktree
        # one file at a time and took 90 seconds before a single verifier ran.
        # Every deliverable then measured its work against a 240s budget that
        # the copy had already spent. robocopy walks the same bytes with
        # sixteen threads; the snapshot is still a full copy, so the source and
        # snapshot fingerprints must still agree.
        clone_command = [
            "robocopy",
            str(ROOT),
            str(snapshot),
...
def _link_directory(link: Path, target: Path) -> None:
    """Point `link` at directory `target`, however this host allows it.

    Windows refuses symlink creation without Developer Mode or
    SeCreateSymbolicLinkPrivilege, but a directory junction needs neither and
    redirects reads and writes the same way. Fall back to one rather than
    leaving the volatile mount unmounted, which would let build artifacts land
    in the workspace and change the very tree this verifier replays against.
    """
    ...
    if result.returncode != 0 or not link.exists():
        raise RuntimeError(
            f"could not link {link} to {target}: {result.stderr.strip()}"
        )
...
def _mount_verifier_volatiles(
    ...
    for relative in (Path("src/cli/node_modules"), Path("node_modules")):
        _link_directory(volatile_root / relative, workspace / relative)
    return mounts

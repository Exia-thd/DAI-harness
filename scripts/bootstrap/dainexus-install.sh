...
# Under MSYS/Git Bash, `pwd` and $HOME yield /c/... while node and the native
# hosts reading the configs written here speak C:/.... Emit the one spelling
# both understand so a written hook path stays recognizable as trusted.
canonical_path() {
...
# DAI Nexus directory
# $HOME arrives as an MSYS path (/tmp/... , /c/...) under Git Bash. Native
# tools called below - node, and the hosts that later read these configs -
# resolve that against the current drive and open the wrong file, so pin it
# to the spelling both understand before deriving any config path from it.
HOME="$(canonical_path "$HOME")"
DAINEXUS_DIR="$(canonical_path "${DAINEXUS_DIR:-$HOME/.dainexus}")"
...
install_rule_context_runtime() {
    ...
    if ! python3 - "$source_manifest" "$source_dir" "$DAINEXUS_DIR" <<'PYEOF'
import json
import shutil
import sys
from pathlib import Path
...
rule_context_command() {
    ...
    resolver+='if [ "${DAINEXUS_RULE_HOOK_MODE:-observe}" = "off" ]; then printf "%s\\n" "{\"continue\":true}"; exit 0; fi; '
    resolver+='hint="${DAINEXUS_PROJECT_ROOT:-${DAINEXUS_WORKSPACE:-}}"; '
    ...
    resolver+='if [ -z "${DAINEXUS_PROJECT_ROOT:-}" ] && [ -z "${DAINEXUS_WORKSPACE:-}" ] && [ -z "$host_hint" ] && command -v git >/dev/null 2>&1; then '
    resolver+='git_root="$(git -C "${candidate:-${PWD:-.}}" rev-parse --show-toplevel 2>/dev/null || true)"; '
    resolver+='case "$git_root" in /*|[A-Za-z]:/*) candidate="$git_root";; esac; fi; '
...
try:
    spec = importlib.util.spec_from_file_location("dainexus_rule_context_hook", sys.argv[1])
...
PYEOF
    ...
    printf 'DAINEXUS_RULE_HOOK_MODE="${DAINEXUS_RULE_HOOK_MODE:-observe}" sh -c %s -- %s %s %s %s || true' \
...
install_hooks() {
    ...
    local context_hook="${DAINEXUS_DIR}/scripts/lite/rule-context-hook.py"
    local context_workspace="${DAINEXUS_DIR}"
    ...
    mkdir -p "$HOME/.claude"
    CLAUDE_SESSION_CMD="$claude_session_cmd" CLAUDE_SUBAGENT_CMD="$claude_subagent_cmd" GATE_SCRIPT="$gate_script" DAINEXUS_DIR="$DAINEXUS_DIR" node - "${HOME}/.claude/settings.json" <<'NODE'
var fs = require('fs');
...
function currentGate(command, platform) {
  // Resolve BOTH sides. The stored command spells the path the way it was
  // written (forward slashes on Windows); path.resolve normalizes separators,
  // so comparing a raw extract against a resolved one never matches there and
  // every install appends a second identical Stop hook.
  var extracted = gateCommand(command, platform);
  if (!extracted) return false;
  return path.resolve(extracted) === path.resolve(process.env.GATE_SCRIPT);
}
function legacyVerifyGate(command, platform) {
  ...
  var framework = path.resolve(process.env.DAINEXUS_DIR);
...
[
  ...
  function ownedContext(commandValue) {
    ...
    if (typeof commandValue !== 'string' || commandValue.indexOf('DAINEXUS_RULE_HOOK_MODE=') !== 0 || commandValue.indexOf('hook_path="$1"; fallback_workspace="$2"; platform="$3"; event="$4";') === -1) return false;
...
NODE
    ...
    mkdir -p "$HOME/.gemini"
    GEMINI_BEFORE_AGENT_CMD="$gemini_before_agent_cmd" GATE_SCRIPT="$gate_script" DAINEXUS_DIR="$DAINEXUS_DIR" node - "${HOME}/.gemini/settings.json" <<'NODE'
var fs = require('fs');
var path = require('path');
var file = process.argv[2];
function atomicWrite(target, content) {
...
var beforeGate = process.env.DAINEXUS_DIR + '/scripts/lite/gemini-before-tool-gate.sh';
...
if (!found) groups.push({ matcher: '*', hooks: [{ name: 'dainexus-rule-context', type: 'command', command: process.env.GEMINI_BEFORE_AGENT_CMD, timeout: 2000 }] });
...
NODE
    ...
    ANTIGRAVITY_PRE_INVOCATION_CMD="$antigravity_pre_invocation_cmd" DAINEXUS_DIR="$DAINEXUS_DIR" node - "${HOME}/.gemini/config/hooks.json" <<'NODE'
var fs = require('fs');
var path = require('path');
var file = process.argv[2];
function atomicWrite(target, content) {
...
var policy = cfg['dai-nexus-policy'];
if (!policy || Array.isArray(policy) || typeof policy !== 'object') policy = {};
if (!Object.prototype.hasOwnProperty.call(policy, 'PreToolUse')) {
  policy.PreToolUse = [{ matcher: '*', hooks: [{ type: 'command', command: 'bash ' + process.env.DAINEXUS_DIR + '/scripts/lite/antigravity-pre-tool-gate.sh', timeout: 5 }] }];
}
...
NODE
    ...
    mkdir -p "$HOME/.cursor"
    CURSOR_SESSION_CMD="$cursor_session_cmd" GATE_SCRIPT="$gate_script" DAINEXUS_DIR="$DAINEXUS_DIR" node - "${HOME}/.cursor/hooks.json" <<'NODE'
var fs = require('fs');
var path = require('path');
var file = process.argv[2];
function atomicWrite(target, content) {
...
NODE
    ...
    if [[ -f "$codex_file" ]]; then
        DAINEXUS_DIR="$DAINEXUS_DIR" python3 - "$codex_file" <<'PYEOF'
import os
import re
import shlex
import sys
...
def owned_legacy_verify(command: str) -> bool:
    ...
    installed = (Path(os.environ["DAINEXUS_DIR"]) / "scripts" / "lite" / "verify-gate.sh").as_posix()
    framework = (Path(os.environ["DAINEXUS_DIR"]).resolve()).as_posix()
...
HOOKBLOCK
    ...
    CONTEXT_SESSION_CMD="$codex_session_cmd" CONTEXT_SUBAGENT_CMD="$codex_subagent_cmd" DAINEXUS_DIR="$DAINEXUS_DIR" python3 - "$codex_file" <<'PYEOF'
import json
import os
import re
import sys
...
def owned_context_command(candidate, expected):
    ...
    if not candidate.startswith("DAINEXUS_RULE_HOOK_MODE=") or marker not in candidate:
        return False
    ...
    framework = Path(os.environ["DAINEXUS_DIR"]).resolve()
